 ...
 * @returns
 */
export const ErrorResponse = (StatusCode: HttpStatus, errorResponseOptions: ErrorResponseOption[]) => {
    ...
    const examples = errorResponseOptions
        .map((error: ErrorResponseOption) => {
            ...
            if (error.model === CustomValidationError) {
                ...
                innerErrorDto = new HttpExceptionErrorResponseDto(StatusCode, error.model.name, error.message, error.code);
            }
                ...
                'application/json': {
                    schema: {
                        ...
                        oneOf: flagValidationErrorExist
                            ? [{ $ref: getSchemaPath(ValidationErrorResponseDto) }, { $ref: getSchemaPath(HttpExceptionErrorResponseDto) }]
